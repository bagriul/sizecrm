            ...
            for entry in data['entry']:
                if 'messaging' in entry:
                    for message_event in entry['messaging']:
                        ...
                        def get_instagram_username(user_id, access_token):
                            ...
                            if response.status_code == 200:
                                data = response.json()
                                return data.get('username', None)  # Return the username or None if not found
                            else:
                                print(f"Error: {response.status_code} - {response.text}")
                                return None

                        sender_username = get_instagram_username(sender_id, config.access_token_instagram)
                        recipient_username = get_instagram_username(recipient_id, config.access_token_instagram)
...
def get_instagram_msg():
    ...
    access_token = data.get('access_token')
    user_id = decode_access_token(access_token, SECRET_KEY).get('user_id')
    recipient_username = users_collection.find_one({'_id': ObjectId(user_id)})['instagram']
